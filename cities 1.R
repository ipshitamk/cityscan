# Read data -------------------------------------------------
# city data folders
# city_folders <- list(sa = 'Sarajevo', ki = 'Kigali')
# city_folders <- list(
#   ki = 'Kigali')
# 
# # city prefixes
# city_prefixes <- names(city_folders)

setwd("/Users/ipshitakarmakar/Dropbox (MIT)/WB/africa-crc-workshop 2")
getwd()
...
for (city in city_prefixes) {
  ...
  if (file.exists(paste0(city_folder, '/osm_infrastructure', 'osm_fire'))) fire[[city]] <- sf::st_read(paste0(city_folder, '/osm_infrastructure'), 'osm_fire')
...
for (city in city_prefixes) {
  ...
  sol_val[[city]] <- if (length(sol[[city]]) == 1) NA else values(sol[[city]])
  sol_col[[city]] <- colorNumeric('PuOr', sol_val[[city]], na.color = 'transparent', reverse = T)
  soc_val[[city]] <- if (length(soc[[city]]) == 1) NA else values(soc[[city]])
  soc_col[[city]] <- colorNumeric('PuOr', soc_val[[city]], na.color = 'transparent', reverse = T)
  pop_val[[city]] <- if (length(pop[[city]]) == 1) NA else values(pop[[city]])
  ...
  temp_val[[city]] <- if (length(temp[[city]]) == 1) NA else values(temp[[city]])
  ...
  air_val[[city]] <- if (length(air[[city]]) == 1) NA else values(air[[city]])
